...
CREATE TABLE `job_announcements` (
    ...
    `recruitment_link` TEXT NULL,
    `recruitment_type` VARCHAR(20) NULL,
    ...
    INDEX `idx_jobs_deadline_at`(`deadline_at`),
    INDEX `idx_jobs_status_updated_at`(`status`, `updated_at`),
    INDEX `idx_jobs_source_channel`(`source_channel`),
    PRIMARY KEY (`id`)
...
CREATE TABLE `service_products` (
    ...
    INDEX `idx_service_products_status_hot`(`status`, `is_hot`, `sales_count`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
