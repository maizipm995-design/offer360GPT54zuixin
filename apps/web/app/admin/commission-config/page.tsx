...
export default function AdminCommissionConfigPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setMessage(error instanceof Error ? error.message : '分销配置加载失败');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage(error instanceof Error ? error.message : '分销配置保存失败');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-card">
        <p className="text-xs font-semibold uppercase tracking-[0.24em] text-brand">Admin commission config</p>
        <h2 className="mt-2 text-3xl font-bold text-ink">分销配置</h2>
        <p className="mt-2 text-sm text-muted">这是后台单例配置页，用于调整一级分销比例，新的订单会按这里的比例结算。</p>
      </section>

      {loading && !config ? <Card className="p-8 text-sm text-muted">正在加载分销配置...</Card> : null}

      {config ? (
        <section className="grid gap-6 xl:grid-cols-[420px_minmax(0,1fr)]">
          <Card className="rounded-3xl p-6">
            <h3 className="text-xl font-semibold text-ink">一级分销比例</h3>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Card className="rounded-3xl p-5">
              <p className="text-sm text-muted">当前一级分销比例</p>
