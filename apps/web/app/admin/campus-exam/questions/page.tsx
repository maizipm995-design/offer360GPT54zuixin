      ...
      <section className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_480px]">
        ...
        <Card className="rounded-3xl p-5 xl:sticky xl:top-6 xl:self-start">
          <div className="flex items-center justify-between gap-3">
            <div>
              <h3 className="text-xl font-semibold text-ink">题目详情 / 编辑</h3>
              <p className="mt-1 text-sm text-muted">JSON 字段支持直接编辑，保存前请确保格式合法。</p>
            </div>
            {detail ? (
          ...
          {detail ? (
            <div className="mt-5 space-y-4">
              ...
              <label className="block space-y-2">
                ...
                <Textarea className="min-h-[140px] font-mono text-xs" value={form.optionsJson} onChange={(event) => setForm((prev) => ({ ...prev, optionsJson: event.target.value }))} />
              </label>
              <label className="block space-y-2">
                <span className="text-sm font-medium text-ink">答案 JSON</span>
                <Textarea className="min-h-[160px] font-mono text-xs" value={form.answerJson} onChange={(event) => setForm((prev) => ({ ...prev, answerJson: event.target.value }))} />
              </label>
            ...
            <p className="mt-5 text-sm text-slate-500">请先从左侧列表选择题目。</p>
          )}
        </Card>
      </section>
    </div>
