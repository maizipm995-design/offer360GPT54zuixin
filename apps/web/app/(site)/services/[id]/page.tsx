...
export default function ServiceDetailPage() {
  ...
  useEffect(() => {
    clientFetch<ServiceItem>(`/service-products/${encodeURIComponent(serviceId)}`)
      .then(setService)
      .catch((error) => setMessage(error instanceof Error ? error.message : '商品加载失败'));
  }, [serviceId]);

  const handleBuy = async () => {
    if (!token) {
      router.push(`/login?redirect=${encodeURIComponent(`/services/${encodeURIComponent(serviceId)}`)}`);
      return;
    }

    setJumpingToCheckout(true);
    try {
      router.push(`/checkout?productId=${encodeURIComponent(serviceId)}`);
    } catch (error) {
  ...
  if (!service) {
    return <main className="mx-auto max-w-[1080px] px-4 py-10">{message || '加载中...'}</main>;
  }
